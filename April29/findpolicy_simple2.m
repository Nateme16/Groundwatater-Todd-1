...
function [policy policyopt v X R wp] = findpolicy_simple2(n,beta,r,k,g,c0,c1,A,rec,S,re,max_k,min_k,tol,maxit)
...
for i = 1:n % loop over the water states;
    ...
    for j = 1:n % loop over next period's water states;
        ...
        if(y <= Gamma(x)&&y>= min_k && y<= max_k && y<x)% && 0<((((x-y) - eom2(rec,re,0,irrig(A,max_k,min_k,x),S)).*(irrig(A,max_k,min_k,x)*S) )./(1-re))./irrig(A,max_k,min_k,x));
            % if so, set the appropriate return and corresponding policy 
        wp(i,j)= ((((x-y) - eom2(rec,re,0,irrig(A,max_k,min_k,x),S)).*(irrig(A,max_k,min_k,x)*S) )./(1-re))./irrig(A,max_k,min_k,x) ;% Policy (per irrigated acre) corresponding to grid space
