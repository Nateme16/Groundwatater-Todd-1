...
function [policy policyopt v X R wp] = findpolicy_yield_stoch(n,beta,r,P,c0,c1,ps,pc,pw,A,rec,S,re,max_k,min_k,tol,maxit,farm,init_k,R_ar,wp_ar)

%% Define payoff space (returns to pumping choice)- commented out since it uses 'R' matrix from findpolicy_AR1 to save time. 
...
for i=1:maxit
    ...
    for j=1:n
        ...
        [tv(j,e) I(j,e)] = max(R(j,:,e) + (beta.* v*P')'); %incorporates state transition probabilities
