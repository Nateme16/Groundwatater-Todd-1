...
for i=1:size(X,2);
    x=X(i);
 policy_myop(i)=fminsearch(@(w) -pi_total_yield(w,r,c0,c1,ps,pc,pw,irrig(A,max_k,min_k,x,farm,init_k),A,x,farm),2);
end
...
for i=1:j;
    ...
    if (x2(i)>=min_k);
    myop(i)=  fminsearch(@(w) - pi_total_yield(w,r,c0,c1,ps,pc,pw,irrig(A,max_k,min_k,x2(i),farm,init_k),A,x2(i),farm),2); 
    ...
    benefitopt(i)=  exp(-(1-beta)*i)*  pi_total_yield(optimw(i),r,c0,c1,ps,pc,pw,irrig(A,max_k,min_k,x(i),farm,init_k),A,x(i),farm);
    benefitmyop(i)=  exp(-(1-beta)*i)* pi_total_yield(myop(i),r,c0,c1,ps,pc,pw,irrig(A,max_k,min_k,x2(i),farm,init_k),A,x2(i),farm);
