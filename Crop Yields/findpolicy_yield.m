...
for i = 1:n % loop over the water states;
    ...
    for j = 1:n % loop over next period's water states;
        ...
        wp(i,j)= (( ((x-y) - eom2(rec,re,0,irrig(A,max_k,min_k,x,farm),S,farm)) .*((irrig(A,max_k,min_k,x,farm)./farm)*S) )./(1-re)) ./ irrig(A,max_k,min_k,x,farm) ;% Policy (per irrigated acre) corresponding to grid space
