...
function [policy policyopt v X R wp] = findpolicy_yield_AR1(n,beta,r,c0,c1,ps,pc,A,rec,S,re,max_k,min_k,tol,maxit,prob,farm)
...
for i = 1:n % loop over the water states;
    ...
    for j = 1:n % loop over next period's water states;
       ...
       % eomtest(i,j,e)=eom2(rec,re,0,irrig(A,max_k,min_k,x),S,farm);
        ...
        R(i,j,e)=pi_total_yield(wp(i,j,e),r(e),c0,c1,ps,pc,irrig(A,max_k,min_k,x,farm),A,x,farm); %profit from choice of policy
        end
        end
