...
for i = 1:n % loop over the water states;
    ...
    for j = 1:n % loop over next period's water states;
        ...
        wp(i,j,e)= ((((x-y) - eom2(rec,re,0,irrig(A,max_k,min_k,x),S)).*((irrig(A,max_k,min_k,x)./.12)*S) )./(1-re))./irrig(A,max_k,min_k,x) ;% Policy (per irrigated acre) corresponding to grid space
