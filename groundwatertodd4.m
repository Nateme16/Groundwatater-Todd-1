...
maxit = 10000; % maximum number of loop iterations

n =51; % n specifies how fine our grid of capital holdings will be
X = linspace(min_x,max_x,n); % an evenly spaced grid
grid=(max_x-min_x)/n
xmat=min_x:grid:max_x
...
while diff>tol & its<maxit;
   ...
   [x1] = fminsearch(@(x)-valfun3(x,n,X,alpha,A,S,rec,re,beta,v0,x0,u1),.5);
   
   [v1(1,i) policy(1,i)] = valfun3(x1,n,X,alpha,A,S,rec,re,beta,v0,x0,u1);
